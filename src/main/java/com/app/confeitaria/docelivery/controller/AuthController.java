...
import com.app.confeitaria.docelivery.dto.CadastroClienteDTO;
...
public class AuthController {
    ...
    public ResponseEntity<?> login(@RequestBody Map<String, String> loginData) {
        ...
        return usuarioRepository.findByEmail(email).map(user -> {
            ...
            // Mapa com dados do usuário para o frontend
            ...
            // "role" explícito — usado pelo frontend para bloquear outros tipos de usuário
            userData.put("role", "ROLE_" + ((user.getTipoUsuario() != null) ? user.getTipoUsuario().name() : "CLIENTE"));
    ...
    public ResponseEntity<?> cadastrarCliente(@Valid @RequestBody CadastroClienteDTO dados) {
        try {
            if (usuarioRepository.findByEmail(dados.email()).isPresent()) {
                return ResponseEntity.status(HttpStatus.CONFLICT)
                        .body(Map.of("error", "E-mail já cadastrado no sistema."));
            }
            ...
            if (dados.sobrenome() != null) cliente.setNome(dados.nome() + " " + dados.sobrenome());
            if (dados.telefone() != null) cliente.setTelefone(dados.telefone());
            if (dados.apelido() != null) cliente.setApelido(dados.apelido());
